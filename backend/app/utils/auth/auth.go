...
func CreateUserSession(w http.ResponseWriter, r *http.Request, u UserSessionInfo) error {
	...
	session.Values["userUid"] = u.UserUid
...
func GetSessionUserId(r *http.Request) (int32, error) {
	myUserId, err := GetSessionData[int32](r, "userId")
	if err != nil {
...
func GetSessionUserUid(r *http.Request) (uuid.UUID, error) {
	myUserUid, err := GetSessionData[uuid.UUID](r, "userUid")
	if err != nil {
		log.Printf("failed to retrieve user uid from session data: %v", err.Error())
	}
	return myUserUid, err
}
...
func GetUserSessionInfo(r *http.Request) (UserSessionInfo, error) {
	...
	sessionInfo = UserSessionInfo{
		...
		UserUid:         session.Values["userUid"].(uuid.UUID),
		UserId:          session.Values["userId"].(int32),
